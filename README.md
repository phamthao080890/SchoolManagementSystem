...
This repository includes [`render.yaml`](render.yaml), which creates:

- `school-management-api` as a Node.js web service.
- `school-management-client` as a Vite static site, with an SPA rewrite to `index.html`.

Create a new **Blueprint** in Render from the repository. During its initial setup, enter values for the requested secrets:

- `MONGO_URI`: a MongoDB Atlas connection string reachable from Render.
- `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_BUCKET_NAME`: required for file uploads. They can be left unset only if the upload feature will not be used.

`JWT_SECRET` is generated by Render. The static client is built with `VITE_API_URL=https://school-management-api.onrender.com`, and the API permits `https://school-management-client.onrender.com` via `CLIENT_ORIGIN`.

If either Render service name is changed (or custom domains are used), update all three matching values in `render.yaml` before deploying:

- client `VITE_API_URL`
- API `CLIENT_ORIGIN`
- API `API_BASE_URL`

The API's Render start command is `npm run start:render`, which runs `npm run seed` before starting Express. The current seed script deliberately clears and recreates its collections each time the web service starts, so do not use this setting with a database that contains production data you need to keep.
